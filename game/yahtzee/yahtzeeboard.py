...
class YahtzeeBoard:
    ...
    def rollDiceListener(self):
        ...
        for i in range(5):
            if not self.diceButtons[i]['state'] == 'disabled':
                self.dice[i].rollDie()
                self.diceButtons[i].configure(text=self.dice[i].getRoll())
    ...
    def diceListener(self, row):
        if not self.diceButtons[row]['text'] == '?':
    ...
    def categoryListener(self, row):
        ...
        # (3) 버튼을 disable 시킴.
        # TODO: 구현
        cur_player.setScore(score, row)
        cur_player.setAtUsed(row)
        self.fields[row][self.player].configure(text=score)
        ...
        if cur_player.allUpperUsed():
            self.fields[self.UPPERTOTAL][self.player].configure(text=cur_player.getUpperScore())
        ...
        if cur_player.allLowerUsed():
            self.fields[self.LOWERTOTAL][self.player].configure(text=cur_player.getLowerScore())

        # UPPER category와 LOWER category가 전부 사용되었으면 
        # -> UI의 TOTAL 에 내용 채우기.
        # TODO: 구현
        if cur_player.allUpperUsed() and cur_player.allLowerUsed():
            self.fields[self.TOTAL][self.player].configure(text=cur_player.getUpperScore() + cur_player.getLowerScore())
        ...
        for i in range(self.TOTAL - 1):
            for j in range(self.numPlayers):
                if i == 6 or i == 7:
                    ...
                    self.fields[i][j]['state'] = 'disabled'
                    self.fields[i][j]['bg'] = 'light gray'
                elif self.players[self.player].used[i]:
                    self.fields[i][j]['state'] = 'disabled'
                    self.fields[i][j]['bg'] = 'light gray'
